...
use "${pathCOMPUSTAT}\COMPUSTAT_RAW_20200706.dta", clear 
*Compustat North America Daily as of 07/06/2020, via WRDS

{	//Duplicates & data flaws
*CUSIP as identifier for matching SDC data 
	drop if missing(cusip)	//check since WRDS-download was conditional on non-missing CUSIP
...
*indfmt datafmt popsrc consol curcd
	...
	drop if missing(cik) 		//furhter data cleansing, ensures that firm is publicly traded under the regulations of the SEC
...
{	//DELTA_ETRs
	*DELTA_CASH_ETR
	gen DELTA_CASH_ETR1 = CASH_ETR1/LAG_CASH_ETR1_1
		replace DELTA_CASH_ETR1 = 0 if CASH_ETR1 == 0 & LAG_CASH_ETR1_1 == 0
	...
	gen DELTA_CASH_ETR3 = CASH_ETR3/LAG_PRE_CASH_ETR3_1
		replace DELTA_CASH_ETR3 = 0 if CASH_ETR3 == 0 & LAG_PRE_CASH_ETR3_1 == 0
	...
	gen DELTA_CASH_ETR5 = CASH_ETR5/LAG_PRE_CASH_ETR5_1
		replace DELTA_CASH_ETR5 = 0 if CASH_ETR5 == 0 & LAG_PRE_CASH_ETR5_1 == 0
...
{	//2-digit Industry Cluster
	...
	* 12 = Nonclassifiable Establishments --> 99
	...
	replace IND = .  if missing(twosic)	//check 
...
{	//Annual Measures
	...
	*Create Leads
	sort CUSIP6 fyear
	foreach var of varlist ebitda_at RnDExp AdExp SGA CapEx ChangeSale Leverage Cash Intangibles PPE size MNE NOL{
		forvalues q = 1(1)5{
			by CUSIP6: gen LEAD_`var'_`q' = `var'[_n+`q']
		}
	}
...
{	//Industry-mean-adjusted effective tax rates 
	
	//Determine mean(ETR) by industry & year 
	sort IND fyear CUSIP6  
	foreach var of varlist CASH_ETR* GAAP_ETR* PRE_CASH_ETR* PRE_GAAP_ETR*{
		by IND fyear: egen indyearmean_`var' = mean(`var')
		}
		
	//Adjusted ETRs 
	sort IND fyear CUSIP6  
	foreach var of varlist CASH_ETR* GAAP_ETR* PRE_CASH_ETR* PRE_GAAP_ETR*{
		by IND fyear: gen indyearadj_`var' = `var' - indyearmean_`var'
		}

	//LEADS & LAGS of industry-year-mean-adjusted ETRs 
	*LEADS
	sort CUSIP6 fyear
	foreach var of varlist indyearmean_* indyearadj* {
		forvalues q = 1(1)5{
			by CUSIP6: gen LEAD_`var'_`q' = `var'[_n+`q']
		}
	}
	...
	*LAGS
	...
	sort CUSIP6 fyear
	*graph box LAG_indyearadj_PRE_CASH_ETR3_1, over(fyear)
}
